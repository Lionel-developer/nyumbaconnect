...
export default function EditPropertyPage() {
  ...
  // ✅ stable first render to prevent hydration mismatch
  ...
  const refreshProperty = useCallback(async () => {
    // ✅ correct endpoint (matches your routes: router.get('/:id', optionalProtect, getPropertyById))
    const res = await api.get(`/api/properties/${id}`);
    const p = res.data?.data?.property || null;
    const visibility = res.data?.visibility || "public";

    // If backend says not owner, block editing here
    if (visibility !== "owner") {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // If backend returns 403/401, show clear message
        if (String(msg).toLowerCase().includes("forbidden")) {
          setError("Forbidden: you can only edit your own listings.");
        } else {
          setError(msg);
        }
      } finally {
  ...
  }, [authReady, auth.token, id, isLandlordOrAgent, refreshProperty, router]);
  ...
  const onSave = async (e) => {
    ...
    try {
      ...
      // ✅ correct endpoint (routes: router.put('/:id', protect, requireRole(...), updateProperty))
        ...
        <form onSubmit={onSave} className="mt-4 rounded-2xl bg-[var(--taupe-300)] p-4 sm:p-5">
          <div className="grid gap-3">
            ...
            <div className="mt-2 rounded-2xl bg-neutral-300 p-4">
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                <label className="block">
                  ...
                  <p className="mt-1 text-xs opacity-70">
                    Upload photos from your phone (best quality).
                  </p>
                </label>
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs opacity-70">Must start with http:// or https://</p>
                </div>
              </div>
              ...
              {!images?.length && (
                <p className="mt-3 text-sm opacity-70">No images yet. Upload one to get started.</p>
              )}
            </div>
            ...
            <button
              type="button"
              className="rounded-2xl bg-white px-4 py-3 font-semibold shadow-sm ring-1 ring-black/10"
